...
## 2026-06-30 — T-081: Venues — galleries / shops as discovery destinations

Galleries and shops as a parallel supply side for the discovery story.
Independent artists often don't have a studio worth visiting; venues
fill that gap.

Schema (`0025_venues.sql`):

- `venues` table — top-level entity. id, slug, name, kind
  (gallery / shop / studio_collective / cafe_collab / other),
  address fields, lat/lng/city/country (geocoded async), website
  + Instagram + opening_info, owner_user_id, status (default
  `pending_review`).
- `venue_artworks` junction — (venue_id, artwork_id) PK + status
  (pending / accepted / declined) + requested_at + decided_at.
  artwork_id FK ON DELETE CASCADE.
- Indexes: unique slug (partial on non-deleted), by-owner, active
  map-pin scan, per-artwork accepted, status+requested_at.

Backend (`api-search::venues`):

- Studio CRUD: POST/GET/PATCH/DELETE `/v1/studio/venues[/:id]`
  with ownership enforced inline.
- Invitation flow: POST/DELETE
  `/v1/studio/venues/:venue_id/artworks/:artwork_id`. ON CONFLICT
  re-invitation flips a `declined` row back to `pending`.
- Artist-side: GET `/v1/studio/venue-requests` + POST
  `/v1/studio/venue-requests/:venue_id/:artwork_id/{accept,decline}`.
- Public reads: `/v1/venues` (paginated, optional `?city`),
  `/v1/venues/:slug` (404 for non-active). Public detail includes
  accepted artworks with primary image URLs.
- New `JobEvent::VenueGeocode` + `geocode_and_update_venue` mirror
  the artist-location geocoding path.
- `ArtworkFull.venues: Vec<ArtworkVenueRef>` — accepted venues
  surface on `/v1/artworks/:id` so the artwork detail page can
  render a "Currently at" strip.

Web:

- `/studio/venues` — owner's venue list with status badges +
  URL-driven `VenueEditModal` (two tabs: Details + Artworks).
  Artwork tab loads existing invitations + has a paste-an-id input
  to invite. Search-to-invite is a deferred follow-up.
- `/studio/venue-requests` — artist's pending-invitation inbox
  with Accept / Decline buttons (decline goes through useConfirm).
- `/venues` — public index + `/venues/[slug]` detail with the
  "Currently showing" grid of accepted artworks.
- Artwork detail page renders the "Currently at" strip below the
  description when at least one venue accepted the work.

Admin (T-081.4):

- GET `/v1/admin/venues?status=…` + POST `/v1/admin/venues/:id/
  {approve,decline}` using the same `transition` shape as
  artists. Writes `admin_audit_log` per mutation (action
  `venue.approve` / `venue.decline`).
- `/admin/venues` page with the same tab-strip + per-row
  affordance pattern as `/admin/artists`. `/admin` index gains a
  fourth tile counting pending venues.

Tests: 20 integration tests in `venues_test.rs` covering studio
CRUD, ownership boundaries, slug collision, invitation flow,
re-invite reopens pending, cascade-clear on artwork delete,
public visibility gates, and the four admin transitions. Plus
one in `artwork_test.rs` asserting `ArtworkFull.venues` populates
correctly.
